...
def list_notifications(db: Session = Depends(get_db_dep), user_id: str = Depends(get_current_user_id)):
    return EmailService(db).list_notifications(user_id)

@router.post("/notifications", response_model=NotificationOut)
def create_notification(payload: NotificationCreate, db: Session = Depends(get_db_dep), user_id: str = Depends(get_current_user_id)):
